...
class module:
    ...
    error_choices_s_s_empty = (
        "Choices for '%s' argument '%s' is empty. Please check if vectors are declared before arguments."
    )
...
class module_file_archive:
    ...
    error_extracting_s_folder_needed = (
        "Error extracting %s, first remote file argument must contain the output folder path"
    )
...
class module_file_mount:
    httpfs_s_not_found = (
        "HTTPfs binary '%s' not found, specify binary path or install it from 'https://github.com/cyrus-and/httpfs'"
    )
